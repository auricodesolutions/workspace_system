...
class DashboardController {
  ...
  async summary(@Req() request: AuthRequest, @Query("businessUnit") code = "ALD") {
    ...
    const [activeProjects, tasksToday, overdueTasks, atRiskProjects, pendingApprovals, pendingAcceptance, invoices, tasks, activeUsers, walletTotals, clientTotals, accountTotals, renewalReminders] = await Promise.all([
