...
class QuotationsController {
  ...
  async convert(@Req() request: AuthRequest, @Param("id") id: string, @Body() dto: ConvertQuotationDto) {
    return this.prisma.$transaction(async (tx) => { const quotation = await tx.quotation.findFirstOrThrow({ where: { id, organizationId: request.user.organizationId }, include: { items: true, client: { include: { businessUnit: true } } } }); if (quotation.status !== "ACCEPTED") throw new BadRequestException("Only an accepted quotation can be converted"); if (dto.paymentAccountId) await tx.financialAccount.findFirstOrThrow({ where: { id: dto.paymentAccountId, organizationId: request.user.organizationId } }); const count = await tx.invoice.count({ where: { businessUnitId: quotation.businessUnitId } }); const invoiceNumber = `${quotation.client.businessUnit.invoicePrefix}-${String(count + 1).padStart(5, "0")}`; const invoice = await tx.invoice.create({ data: { organizationId: quotation.organizationId, businessUnitId: quotation.businessUnitId, clientId: quotation.clientId, projectId: quotation.projectId, paymentAccountId: dto.paymentAccountId, invoiceNumber, issueDate: new Date(), dueDate: new Date(dto.dueDate), subtotal: quotation.subtotal, discount: quotation.discount, tax: quotation.tax, total: quotation.total, notes: quotation.notes, items: { create: quotation.items.map((item) => ({ description: item.description, unitPrice: item.unitPrice, quantity: item.quantity, discount: item.discount, total: item.total, sortOrder: item.sortOrder })) } } }); await tx.quotation.update({ where: { id }, data: { status: "CONVERTED", convertedInvoiceId: invoice.id } }); await tx.auditLog.create({ data: { organizationId: request.user.organizationId, actorId: request.user.id, action: "CONVERT", entityType: "QUOTATION", entityId: id, summary: `Converted ${quotation.quotationNumber} to ${invoice.invoiceNumber}` } }); return invoice; });
  }
}
